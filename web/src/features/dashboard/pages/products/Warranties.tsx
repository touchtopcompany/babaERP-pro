...
const Warranties: React.FC = () => {
  ...
  const defaultWarranties: WarrantyData[] = [
    {
      key: "1",
      name: "Standard Warranty",
      description: "Standard manufacturer warranty covering defects",
      duration: "12 months",
    },
    {
      key: "2",
    ...
    },
    {
      ...
      description: "Premium warranty with full coverage and support",
      duration: "36 months",
    },
    {
      key: "4",
    ...
    },
    {
      ...
      description: "Lifetime warranty for the product",
      duration: "Lifetime",
    },
  ];

  const [warranties, setWarranties] = useState<WarrantyData[]>(defaultWarranties);

  // Filter warranties based on search text
  const filteredWarranties = useMemo(() => {
    if (!searchText) return warranties;
    ...
    return warranties.filter(
      (warranty) =>
        ...
        warranty.duration.toLowerCase().includes(searchLower)
    );
  }, [warranties, searchText]);
  ...
  const handleRefresh = () => {
    setLoading(true);
    // TODO: Implement API call to refresh warranties
    setTimeout(() => {
      setLoading(false);
      message.success("Warranties refreshed successfully");
    }, 1000);
  };

  const handleExportCSV = () => {
    const headers = ["Name", "Description", "Duration"];
    const csvData = filteredWarranties.map((warranty) => [
      warranty.name,
    ...
    message.success("Warranties exported to CSV successfully");
  };
  ...
  const handlePrint = () => {
    ...
    const tableHTML = `
      <html>
        ...
        <body>
          <h1>Warranties Management Report</h1>
  ...
  const columns: ColumnsType<WarrantyData> = [
    {
      title: "Name",
      dataIndex: "name",
      key: "name",
  ...
  return (
    <div>
      ...
      >
        ...
        <style>{`
          .warranties-table .ant-table-thead > tr > th {
            transition: all 0.2s ease;
            cursor: pointer;
          }
          .warranties-table .ant-table-thead > tr > th:hover {
            background: ${isDark ? "rgba(24, 144, 255, 0.3)" : "rgba(24, 144, 255, 0.9)"} !important;
          }
          .warranties-table .ant-table-tbody > tr {
            transition: all 0.2s ease;
          }
          .warranties-table .ant-table-tbody > tr:hover > td {
            background: ${isDark ? "rgba(24, 144, 255, 0.1)" : "rgba(24, 144, 255, 0.05)"} !important;
          }
          .warranties-table .ant-table-tbody > tr:hover {
            transform: translateY(-1px);
            box-shadow: ${isDark ? "0 2px 8px rgba(0,0,0,0.2)" : "0 2px 8px rgba(0,0,0,0.08)"};
          }
          .warranties-table .ant-table-column-sorter {
            color: ${isDark ? "rgba(255,255,255,0.65)" : "rgba(0,0,0,0.45)"} !important;
          }
          .warranties-table .ant-table-column-sorted .ant-table-column-sorter-up,
          .warranties-table .ant-table-column-sorted .ant-table-column-sorter-down,
          .warranties-table .ant-table-column-sorter-up.on,
          .warranties-table .ant-table-column-sorter-down.on,
          .warranties-table .ant-table-column-sorter-up.active,
          .warranties-table .ant-table-column-sorter-down.active {
            color: ${isDark ? "#fff" : "#1890ff"} !important;
            opacity: 1 !important;
          }
        `}</style>
        <Table
          className="warranties-table"
      ...
      <AddModal
        ...
        onSave={async (values) => {
          ...
          try {
            // TODO: Implement API call to create warranty
            ...
            message.error("Failed to create warranty");
          } finally {
        ...
        title="Add Warranty"
        fields={[
          { name: "name", label: "Name", type: "text", required: true, placeholder: "Enter warranty name" },
          { name: "description", label: "Description", type: "textarea", required: false, placeholder: "Enter description" },
          { name: "duration", label: "Duration", type: "text", required: false, placeholder: "Enter duration (e.g., 12 months)" },
        ]}
      ...
      <EditModal
        ...
        onSave={async (_values) => {
          setActionLoading(true);
          try {
            // TODO: Implement API call to update warranty
            await new Promise((resolve) => setTimeout(resolve, 1000));
      ...
      <DeleteModal
        ...
        onConfirm={async () => {
          setActionLoading(true);
          try {
            // TODO: Implement API call to delete warranty
            await new Promise((resolve) => setTimeout(resolve, 1000));
            message.success("Warranty deleted successfully");
            setWarranties(warranties.filter((w) => w.key !== selectedWarranty?.key));
            setDeleteModalOpen(false);
            setSelectedWarranty(null);
          } catch (_error) {
        ...
        itemName={selectedWarranty?.name}
        loading={actionLoading}
      />
    </div>
  );
